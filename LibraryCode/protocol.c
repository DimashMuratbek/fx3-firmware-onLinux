 ...
 */
	 ...
	 uint8_t *data, size_t length, size_t sample_width)
 {
	 ...
	 for (i = 0; i < length; i++) {
		//  devc->logic_buffer[i] = data[i * 2];
		 ...
		 devc->analog_buffer[i] = (data[i] / 255.0f) * 3.3f;  // 0–3.3V range	 };
	...
	//  sr_analog_init(&analog, &encoding, &meaning, &spec, 1);  //2
 ...
 static void LIBUSB_CALL receive_transfer(struct libusb_transfer *transfer)
 {
	 ...
	 unitsize = devc->sample_wide ? 4 : 1;  // 4 bytes per sample for 32 bit, 2 for 16 bit

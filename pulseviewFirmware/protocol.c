...
#define NUM_DIGITAL_BYTES 2
 ...
 #define SAMPLE_STRIDE (NUM_DIGITAL_BYTES + 1)
 ...
 static int command_start_acquisition(const struct sr_dev_inst *sdi)
 {
	 ...
	 printf("Sample rate: %zu " , samplerate);
	 ...
	 uint8_t *data, size_t length, size_t sample_width)
 {
	...
	// if (length % 3!= 0) {
	// 	fprintf(stderr, "Misaligned buffer! data_length = %zu\n", length);
	// }
	 ...
	 size_t num_enabled = g_slist_length(devc->enabled_analog_channels);
	printf("Buffer #%zu, \n", length);
	uint8_t ch;
	...
	for (i = 0; i < length; i++) {
    // First byte is digital
    //devc->logic_buffer[i] = data[i * 9];
	uint8_t digital_0 = data[i * SAMPLE_STRIDE + 0];  // first byte (D0 - D7)
    uint8_t digital_1 = data[i * SAMPLE_STRIDE + 1]; // second byte (D8 - D15)
	// uint8_t digital_2  = data[i * SAMPLE_STRIDE + 2];
    // uint8_t digital_3 = data[i * SAMPLE_STRIDE + 3];

    devc->logic_buffer[i * 2 + 0] = digital_0;
    devc->logic_buffer[i * 2 + 1] = digital_1;

    // Next 8 bytes are analog channels
    for (ch = 0; ch < 1; ch++) {
        uint8_t raw = data[i * SAMPLE_STRIDE + NUM_DIGITAL_BYTES + ch];
        float voltage = (raw / 255.0f) * 3.3f;
        devc->analog_buffer[i * num_enabled + ch] = voltage;
	}
	// fprintf(stderr, "Sample %zu: D0 = 0x%02X, D1 = 0x%02X, A0 = %.2f\n", i, digital_0, digital_1, devc->analog_buffer[i]);
    	
	//fprintf(stderr, "Sample %zu: Digital byte = %02X\n", i, data[i * 9]);
	//fprintf(stderr, "Sample %zu: Analog A0 = %.2f\n", i, devc->analog_buffer[i * 8 + 0]);

	}

	// const size_t slice_size = 3;  // 2 bytes digital + 1 byte analog
    // size_t sample_count = length / slice_size;

	
    // for (i = 0; i < sample_count; i++) {
    //     size_t base = i * slice_size;

    //     // Parse digital data
    //     uint16_t digital = data[base] | (data[base + 1] << 8);
    //     devc->logic_buffer[i * 2]     = digital & 0xFF;
    //     devc->logic_buffer[i * 2 + 1] = (digital >> 8) & 0xFF;

    //     // Parse analog data (convert 8-bit to float, assuming 0–255 → 0.0–3.3V for example)
    //     uint8_t analog_raw = data[base + 2];
    //     devc->analog_buffer[i] = (analog_raw / 255.0f) * 3.3f;
    // }



	printf("the simple width: %zu" , sample_width);
 ...
 static void LIBUSB_CALL receive_transfer(struct libusb_transfer *transfer)
 {
	 ...
	 unitsize = devc->sample_wide ? 2 : 1;  // 4 bytes per sample for 32 bit, 2 for 16 bit
 ...
 static int start_transfers(const struct sr_dev_inst *sdi)
 {
	...
	//  printf("Expected FX3 buffer size: %zu bytes\n", size); // ********************
 ...
 SR_PRIV int cypress_fx3_start_acquisition(const struct sr_dev_inst *sdi)
 {
	 ...
	 //printf("The size value: %zu bytes\n", size);

	 size_t expected_samples = size / SAMPLE_STRIDE;

	 /* Prepare for analog sampling. */
	 if (g_slist_length(devc->enabled_analog_channels) > 0) {
		 
		/* get sample one digital byte and 8 analog bytes */
		
		
		devc->logic_buffer = g_try_malloc(expected_samples * 2); 
		
		devc->analog_buffer = g_try_malloc(sizeof(float) * expected_samples);
